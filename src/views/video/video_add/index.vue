<template>
  ...
  <div class=" m-10" v-show="current===0">
    ...
    <a-form :model="video" name="basic" :label-col="{ span: 6 }" :wrapper-col="{ span: 15 }" autocomplete="off"
            @finish="" @finishFailed="onFinishFailed">
      ...
      <a-form-item label="分类" name="categoryId">
        <a-select v-model:value="video.categoryId" placeholder="分类">
          <a-select-option v-for="item in categories" :value="item.id">{{ item.name }}</a-select-option>
        </a-select>
      </a-form-item>

      <a-form-item label="发布时间" name="categoryId">
        <a-date-picker v-model:value="video.publishAtValue" />
      </a-form-item>
      ...
      <a-form-item :wrapper-col="{ offset: 8, span: 16 }">
        <a-button type="primary" html-type="submit" @click="finishVideoInfo">提交</a-button>
      </a-form-item>
    </a-form>
  ...
  audioElement.addEventListener("loadedmetadata", function(_event) {
    ...
    oss().then((res) => {
      ...
      put(action.file.name, action.file, res).then((res) => {
        ...
        cur.editable = true
        cur.url = res.url
        cur.duration = parseInt(duration)
        cur.loading = "上传成功"
      }).catch((e) => {
